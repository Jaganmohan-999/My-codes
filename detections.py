...
MAX_VIDEO_SECONDS = 60          # 2 minutes
...
def _get_model_name(payload: dict) -> str:
    model = payload.get("model", "unknown")
    if isinstance(model, dict):
        return model.get("name", "unknown")
    return str(model)   # already a string like "yolov8_b16"
...
try:
    while True:
        ...
        detections = payload.get("inference", {}).get("detections", [])

        if camera_id != "69c0dd80ecf2d47d54ecad2e":
            #log.info("camera id not matched to 69c9fa6abfdb061267e05a63")
            continue
        ...
        try:
            ...
            log.info(f"[{camera_id}] processing frame: {payload.get('frame_id')}, detections: {detections}")

            raw = base64.b64decode(payload["data"])
            arr = np.frombuffer(raw, dtype=np.uint8).copy()
            frame = cv2.imdecode(arr, cv2.IMREAD_COLOR)  # returns BGR natively — correct for preprocess_yolo
            if frame is None:
                raise ValueError(f"cv2.imdecode failed | camera={camera_id}")

        except Exception as e:
            log.error(f"[{camera_id}] Image reading failed: {str(e)}")
            continue
        ...
        # draw frame_id
        # cv2.putText(vis, payload.get('frame_id'), (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

        fps = payload.get("fps") or FPS
        model = _get_model_name(payload=payload)
        log.info(f"[{camera_id}] fps={fps}, model={model}")
